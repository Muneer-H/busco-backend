...
// Utility type to extract only property keys (not methods)
type ClassProperties<T> = {
  [K in keyof T]: T[K] extends Function ? never : K;
}[keyof T];

type UpdateFields<T> = {
  [K in ClassProperties<T>]?: T[K] | (() => string);
};
...
export abstract class SimpleRepository<T extends BaseEntity> {
  ...
  protected PrepareParams(
    ...
    if (Array.isArray(whereParams)) {
      return whereParams.map((w) =>
        normalize(w as Record<string, unknown>),
  ...
  public async Save(instance: Partial<T>): Promise<T> {
  ...
  public async Upsert(instance: T | T[], conflictCols: string[]) {
